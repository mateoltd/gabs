...
import { LocalWorkerHost, type LocalWorkerFactory } from "../local/worker";
import {
...
function session(
  ...
  async function installReleaseSet(
    ...
    const available = new Map(
      availableLocalModules(data, runtime.catalog).map((m) => [m.id, m]),
    );
    ...
    try {
      ...
      for (const choice of ordered) {
        ...
        const fromVersion = localSchemaVersion(data, module, runtime.catalog);
        const previous = prior?.releases[prior.version];
        const source = previous
          ? hydrateModule(moduleContract(previous.package.artifact))
          : runtime.catalog.bundled.find((m) => m.id === module.id);
  ...
  const current: LocalSession = {
    ...
    setServiceAccess(consumerId, service, allowed) {
      return enqueue(async () => {
        ...
        const choice = localServiceAccess(data, runtime.catalog).find(
    ...
    setReferenceAccess(consumerId, providerId, resource, allowed) {
      return enqueue(async () => {
        ...
        const candidate = localReferenceAccess(data, runtime.catalog).find(
          (item) =>
    ...
    uninstall(moduleId) {
      return enqueue(async () => {
        ...
        if (
          availableLocalModules(data, runtime.catalog).some(
    ...
    execute(module, call, options = {}) {
      ...
      const task = tail.then(async () => {
        ...
        const services =
          call.action === "operation"
            ? serviceContext(data, module, vault.id, runtime.catalog)
            : {
                participants: [],
                artifacts: {},
                referenceArtifacts: {},
...
export function planLocalInstallation(
  ...
  const issue = localReleaseIssue(data, root, catalog);
  if (issue) throw Error(issue);
  const installed = availableLocalModules(data, catalog);
  ...
  const selected = resolveReleaseSet(
    ...
    [...candidates.values()].filter(
      (m) => !localReleaseIssue(data, m, catalog),
    ),
    "1.0.0",
    "1.0.0",
...
export function localSchemaVersion(
  data: LocalData,
  module: ModuleDefinition,
  catalog: ModuleCatalog,
) {
  ...
  const previous = prior
    ...
    : catalog.bundled.find((m) => m.id === module.id);
  return (
    prior?.schemaVersion ??
    (previous
...
export function localReleaseIssue(
  ...
  let version = localSchemaVersion(data, module, catalog);
