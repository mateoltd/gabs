...
export function createQuickUnlockEngine(store: LocalVaultStore) {
  ...
  async function localUnlockStatus(
    id: string,
  ...
  async function configureLocalUnlock(
    id: string,
    password: string,
    ...
    return store.exclusive(id, async () => {
      ...
      await store.update(id, (value) => {
        const stored = current(value, vault);
        signal?.throwIfAborted();
        return { ...stored, unlock, revision: (stored.revision ?? 0) + 1 };
      });
      store.changed(id);
    });
  }

  async function unlockLocalVault<T>(
    id: string,
  ...
  async function updateAttempts(
    ...
    return store.update(base.id, (stored) => {
      if (
        !stored ||
        stored.removedAt !== undefined ||
  ...
  return { localUnlockStatus, configureLocalUnlock, unlockLocalVault };
}
