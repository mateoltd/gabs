-- Forward-compatible local device request runtime; retained v1 releases stay valid.
...
BEGIN
 ...
 SELECT EXISTS(SELECT 1 FROM jsonb_each(coalesce(artifact->'operations','{}'::jsonb)) op WHERE op.value->>'policy'='local') OR coalesce(artifact->'localStorage'->'migrations','{}'::jsonb) <> '{}'::jsonb,
 ...
 IF has_local AND (coalesce(artifact->'local'->>'format','') NOT IN ('suite-local-v1','suite-local-v2')
 ...
 THEN RAISE EXCEPTION 'Local operations and migrations require a signed local executable bundle'; END IF;
 IF NOT has_local AND artifact ? 'local' THEN RAISE EXCEPTION 'Local executable has no declared local operations or migrations'; END IF;
