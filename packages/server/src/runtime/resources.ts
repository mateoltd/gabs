...
import { authorizeReferenceTarget } from "./references";
import {
...
import { type Context, checkModule, lockKey } from "../identity/authorization";
...
export async function executeResource(
  ...
  const module =
    definition ??
    (await workspaceModule(tx, ctx.workspaceId, moduleId, ctx.runtime.catalog));
  requireCondition(
    module.id === moduleId,
    403,
    "CAPABILITY_DENIED",
