...
import type { DB, Tx } from "../persistence/database";
import { inWorkspace } from "../persistence/database";
...
export async function provisionWorkspace(
  ...
  const selected = new Map(
    [...new Set(input.modules ?? runtime.preset.moduleIds)].map((id) => [
      id,
      found(
        input.moduleDefaults?.find((m) => m.id === id) ??
          runtime.moduleDefaults?.find((m) => m.id === id) ??
  ...
  for (const grant of input.serviceGrants ?? runtime.preset.serviceGrants) {
