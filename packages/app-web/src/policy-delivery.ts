...
import { ApiError, type SuiteClient } from "@suite/api-client";
import type { Bootstrap } from "@suite/contracts";
import { canUse, type Snapshot } from "@suite/platform";
...
export function snapshotWithPolicy(
  ...
  return {
    ...
    products: canUse(bootstrap, "inventory", "inventory.read")
      ? snapshot.products
      : canUse(bootstrap, "inventory", "inventory.availability.read")
        ? snapshot.products.map(({ onHand, reserved, ...product }) => product)
        : [],
    orders: canUse(bootstrap, "orders", "orders.read") ? snapshot.orders : [],
  };
}
