...
import { buildServerPackage } from "@suite/module-sdk/node/build-server";
import {
  submitRelease,
  reviewRelease,
  stageRelease,
...
if (command === "keygen") {
  ...
  await writeFile(
    ...
    JSON.stringify(modulePackageManifest(name), null, 2) + "\n",
  );
  await writeFile(
  ...
  const { serviceContractSource } =
    await import("@suite/module-sdk/node/service-contracts");
