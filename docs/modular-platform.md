...
**Downloaded page retention.** Generated corporate resource views keep at most 50 recent query pages within a 5 MiB serialized UTF-8 page-entry budget per account/workspace. Offline views show the download time, or explicitly report it unavailable for older copies. Missing or evicted queries require reconnecting. Page eviction preserves drafts, requests, reviews and retained execution contracts; it does not bound all workspace storage. See [scoped cache evidence](verification/resource-cache/README.md). Explicit working-set selection and reference-cache management remain unfinished.
