...
Invitation creation, acceptance, decline and revocation are owned by the server governance service. Shared workspace locking serializes competing responses and revocations; current administrator authority is rechecked after waits and before returning creation receipts. Repeated revocation/decline acknowledges the original transition without duplicate audit or affecting replacement invitations. The form preserves its retry key and details after an uncertain response and provides plain retry guidance. See [invitation transition acceptance](verification/invitation-transitions/README.md). Role stale-edit recovery and invitation-list pagination remain open.
