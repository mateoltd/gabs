...
## Corporate saved-work import foundation

`@suite/client/work-import` provides guarded admission of the existing `module-work-recovery` export format. It requires a recent account-bound MFA session, current workspace/module permissions and signed original contracts, then retains an exact copy in `ModuleStorage.recoveryImports`. Imported state is separate from executable journals and authoritative receipts; retries are idempotent and unresolved copies prevent ordinary offline-storage removal. A host must supply active-profile/unlock/consent and cancellation guards. This API is not yet connected to product file selection or reviewed restoration. See the [limits and required continuation](verification/corporate-work-import/README.md).
