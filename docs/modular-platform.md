...
Organization policy is a DAG over roles with a protected `Administrador` root, multiple parents, opt-in inheritance, group grants and explicit denials. Validation rejects cycles, orphans and root denials. The editor supports dragging, zoom, a minimap and layered layout. The central permission matrix and module-filtered inspector use the same policy calculation and show permission sources. Independent role tags apply grants and denials to selected roles through the same evaluator; existing group labels remain metadata. The SDK owns the canonical organization schema and derived types. The editor supports draft tag creation, search, role selection, permission decisions, rename and removal; Save organization applies the complete versioned policy. Older clients omitting tags preserve existing assignments. Groups and tags also declare optional module policies. These follow role membership and opt-in inheritance, resolve dependencies, and share authoritative seat checks with direct member assignment. Direct grants remain independent; removing a policy cannot remove an independent grant. Invitations and member edits reconcile policies atomically; unavailable publication or billing leaves intended policy visible without granting runtime access. People & access explains saved sources and edits direct grants separately. See [module-policy acceptance](verification/module-policies/README.md). A durable registry revision and tenant adoption cursor now reconcile approved policy intent through authenticated bootstrap and policy delivery after signed releases change dependencies. Current-policy reads reject obsolete derived access immediately; connected People views refresh their assignment and role queries. Pins and independent direct grants remain intact. Existing pending intent does not block unrelated edits, while new assignments still require capacity and resolvable releases. [Release-transition acceptance](verification/module-policy-releases/README.md) records the local scope. The shared searchable choice control filters groups and independent role tags. The chart and minimap highlight matching roles while retaining the complete hierarchy; text counts and accessible descriptions explain the result. Draft membership/name changes update the preview, while Save organization remains the only policy mutation. See [classification acceptance](verification/organization-classification/README.md). Group administration now renders one selected editor, with searchable selection, distinct generated names, explicit Allow/Deny/Not set decisions and focus returning to Add group after removal. Groups and tags share the permission editor; existing decisions remain selectable even when absent from the current choices. The 100-group journey preserves all other groups and reporting relationships. See [group-administration acceptance](verification/group-administration/README.md). Employee administration, combined large-role/group/tag organizations and broader release/provider acceptance remain open.

Role reads and creation/edit responses include a revision over the displayed direct grants and name. Role PUT requires that revision and a retry key. Server governance rechecks authority under the workspace lock, rejects stale changes and reauthorizes accepted receipt retrieval. People preserves unsaved choices for explicit reload; the matrix retains uncertain calls for exact retry or current-state review. See [role-edit acceptance](verification/role-edits/README.md). Member role summaries keep their basic shape; older unversioned role PUT calls fail safely.

Invitation creation, acceptance, decline and revocation are owned by the server governance service. Shared workspace locking serializes competing responses and revocations; current administrator authority is rechecked after waits and before returning creation receipts. Repeated revocation/decline acknowledges the original transition without duplicate audit or affecting replacement invitations. The form preserves its retry key and details after an uncertain response and provides plain retry guidance. See [invitation transition acceptance](verification/invitation-transitions/README.md). Invitation-list pagination and broader employee administration remain open.

Member browsing uses canonical workspace member/role contracts and returns `items`, `nextCursor`, matched `total`, `workspaceTotal`, `activeTotal` and active `roleCounts`. Pages default to 20 and are bounded at 100. Server search covers names, email and roles; detail hydration is scoped to the selected page. The same authorized service exposes GET member by ID for conflict recovery outside the visible page. People resets page history on search/tab changes, retains global role counts, and exposes read recovery without substituting stale seat totals. [Member-page acceptance](verification/member-pages/README.md) records behavior and limits.

Invitation browsing uses canonical page/query contracts in `contracts/src/workspaces/invitations.ts`, re-exported through the existing package root. GET invitations returns bounded `items` (20 by default, maximum 100), `nextCursor`, search-matched `total`, `workspaceTotal` and nonexpired `pendingTotal`. Literal case-insensitive email search covers the complete history. Stable database timestamp/ID cursors preserve continuation during newer insertions. People provides previous/next navigation, search reset, visible newly created invitations and explicit read retry. [Invitation-page acceptance](verification/invitation-pages/README.md) records scope and limits.
...
Quit the desktop app and launch its executable with `--rotate-storage-key` to request a new database master key at the next protected-storage access. The utility stages and verifies a separate encrypted generation before main activates its OS-protected key record. Saved corporate operations, standalone vaults, migration receipts and other tables are retained; an interrupted rotation resumes without repeating the flag. This requires enough free space for another encrypted database and SQLite journals. Older executables that cannot read the versioned key record are incompatible after upgrade. Do not edit or rename storage/key files to downgrade. See the [rotation procedure, recovery states and acceptance limits](verification/storage-rotation/README.md). Standalone lost-key and cross-device archive recovery now have [scoped acceptance](verification/local-root-recovery/README.md). Corporate recovery and actual provider/platform acceptance remain unfinished ID-03 work.


## Local-profile backup maintenance

The desktop executable accepts `--backup-local-profiles=<new-file>` after all running instances are closed. Supply the backup passphrase through standard input. This creates a portable encrypted archive of standalone profiles, excluding corporate caches and device unlock wrappers. The archive retains the profiles' original passphrase protection. The [backup procedure](verification/local-profile-backup/README.md) records archive creation. The [additive restore command](verification/local-profile-restore/README.md), `--restore-local-profiles=<archive>`, validates the archive before opening live storage and preserves existing profiles and corporate caches. It reads the archive passphrase from standard input; restored profiles still require their original passphrases. Identical retries preserve newer work. `--recover-local-profiles=<archive>` activates a validated independently keyed store when the original is unreadable, retaining the complete original directory. Startup resumes interrupted activation before UI admission. Exact retries preserve newer work; an explicit `--new-local-recovery` requests another generation from the same archive after a further provider loss. See the [procedure and limits](verification/local-root-recovery/README.md). Registered disposable import/recovery staging is cleaned on restart; unregistered legacy paths are preserved. A restore UI, authenticated corporate recovery and actual provider/platform acceptance remain unfinished.

## Corporate saved-work import and reviewed restoration

Settings offers **Import saved work** after this device enables offline storage. It accepts the existing `module-work-recovery` JSON export format, up to 1 MiB. A recent account-bound MFA session, current workspace/module permissions and signed original contracts authorize admission, inspection, restoration and removal. Exact imported copies remain separate from executable journals and authoritative receipts; unresolved copies prevent ordinary offline-storage removal.

The public `@suite/client/work-import` API separates parsing, authority, retained-copy operations and explicit promotion under `packages/client/src/recovery/import`. The shell owns file selection and confirmations; the client owns persistence and server reconciliation. Host guards cover active profile/workspace, unlock, consent, current permissions and cancellation, including immediately before durable commit.

**Restore for review** confirms an irreversible server settlement for the exact original request: recover its existing accepted result, or permanently stop the uncommitted original before correction. File-supplied outcome and continuation claims never authorize execution. A lost reply leaves the imported copy intact for an exact retry. Matching local work retains its input and reviews; conflicting identities are refused. Unlinked drafts get separate deterministic review slots instead of overwriting ordinary drafts. Linked create/update reviews retain the stopped original request, or become a current-record edit when the server already accepted it. Restoration reloads the target, merges disjoint changes and requires fresh choices for conflicts; an accepted create never becomes another create. Existing reviews and reassigned dependencies are preserved rather than overwritten. Removal deletes only the imported copy, preserving restored work and the original file. Advanced copied collision/continuation choices remain inert and require fresh review. See the [verification scope and remaining corporate backup work](verification/corporate-work-import/README.md).

[Actual recovery-file portability](verification/corporate-work-portability/README.md) now covers browser-to-browser, browser-to-desktop, desktop-to-browser and independently keyed desktop-to-desktop recovery of ordinary drafts and queued create requests. The destination authenticates afresh and receives only the original exported files. [Linked record reviews](verification/imported-record-reviews/README.md) extend all four directions with preserved original identity and current-target reconciliation. Bulk encrypted corporate archives, imported reassignment/continuation graphs and broader source transitions remain tracked separately.

[Imported command dependencies](verification/imported-command-dependencies/README.md) can be reconnected through a parent command's correction review. Importing a dependent before its parent keeps it stopped. File-supplied selections are not applied as approval. Selecting a stopped dependent changes only its prerequisite; it remains cancelled until its own reviewed correction is saved under a new identity. Current cross-module grants and signed contracts apply to selection and commit, and original capture dependencies are retained.

Independent collision-draft imports require an explicit original/reassigned source choice. `promoteSavedWorkImport(options, digest, { draftSource })` captures that typed choice before asynchronous recovery, reads the chosen current target and reconstructs conflicts without copied resolutions. The promotion records the selection; the exact imported envelope retains provenance. Restored independent drafts do not acquire copied journal prerequisites. Linked request reassignment requires its own explicit target review.

Linked reassigned update drafts also use the explicit `draftSource` choice. `importedDraftChoices` supplies typed, non-authoritative suggestions to the host UI. Restoration preserves the server-stopped original identity and prerequisites, records only the newly selected local record target and reconstructs conflicts against current values. Other copied reference remappings require separate recovery; accepted effects cannot be retargeted. Settings includes accepted resource receipts while unfinished scoped work depends on them, allowing those receipts to be exported and independently verified on another device before a correction proceeds. See [linked target recovery](verification/imported-linked-targets/README.md) for acceptance and remaining graph scope.

Request-only update/archive exports with a saved record target use `promoteSavedWorkImport(options, digest, { recordTarget })`. `SavedWorkImportChoice` permits either `draftSource` or `recordTarget`; `importedRequestTargets` supplies non-authoritative suggestions. The private target helper validates copied mappings before settlement, reads the freshly selected current record and preserves the exact original call and dependencies. Accepted originals cannot be reassigned. Existing local target decisions and saved reviews, including reviews appearing during the target read, prevent replacement. Restored requests remain held until prerequisite receipts are verified and require their own update comparison or archive confirmation. See [request target recovery](verification/imported-request-targets/README.md). Mixed reference graphs, multiple snapshots and broader failure transitions retain separate acceptance gates.

Request imports also retain `importedReferenceHints` for their subsequent correction form. These deduplicated observations do not rewrite request inputs, saved command input or dependency identities. The explicit record selector handles its own target observation separately. Contradictory destinations are refused before settlement; accepted originals receive no new hints. A stopped request displays the original and suggested IDs, retains missing-prerequisite holds and requires a fresh correction under current authority. Existing local reviews cannot be replaced by imported hints. Draft reference context is described below; multiple-snapshot reconciliation retains its separate gate. See [reference-hint acceptance](verification/imported-reference-hints/README.md).


Draft imports retain the same reference observations alongside their fresh record-target choice. For linked updates, a saved `recordRecovery` target also provides provenance when explicit `createRecovery` observations were absent; it does not establish that any field references that record. The existing recovery guards still apply. Fields remain unchanged until explicitly edited, and accepted originals receive hints only in a separate draft review. Ordinary independent edits read the current target and recompute conflicts instead of restoring copied conflict approvals. Source files remain immutable. See [draft-reference acceptance](verification/imported-draft-references/README.md) for the precise tested scope and remaining reconciliation work.


Imported copies can explicitly replace a current review of the same exact request. `inspectSavedWorkImport` exposes the authorized current snapshots and their content fingerprint. Passing that fingerprint as `SavedWorkImportChoice.replaceReview` retains the current work in `recoveryImports` and switches the live review atomically. Stale fingerprints, capacity exhaustion, identity collisions and already corrected originals prevent replacement. Original calls and current local prerequisite selections remain unchanged. Replaced promotions expose `replacedAt`, allowing another explicit restoration; active-promotion replay stays idempotent.

`importedDraftInput` recovers unresolved competing fields from saved comparison provenance. Their historical base values rebuild current conflicts without treating the conflict form's displayed remote values as user edits or copying approval choices. See [snapshot-switching evidence](verification/imported-snapshots/README.md) for precise coverage.
...
These local incident/recovery records contain no business input or credentials and are not tamper-proof server audit logs. Packaged macOS verifies its bundle seal for post-sign native code; other inventoried assets use build digests. Runtime checks now run every minute and on resume. Detected failures gate all native IPC, cancel transport/capabilities and close access; saved requests retain their original identities for recovery. Read-only `--inspect-integrity`, sanitized `--export-integrity=<new-absolute-file>` and explicit `--repair-integrity` now provide support without opening business storage. Repair retains original records, blocks startup while pending, resumes interrupted publication and requires fresh sign-in. The startup failure dialog exposes export and eligible repair actions. [Procedure, acceptance and limits](verification/integrity-support/README.md). Operational audit delivery and actual signed-platform/provider/physical acceptance remain ID-04 work. [Runtime acceptance](verification/integrity-runtime/README.md). [Implementation evidence and limits](verification/integrity-startup/README.md).


### Administration when a release is unavailable

`PlatformState.unavailableModules` separates known release-selection failures from the valid executable definitions in `modules`. The Modules page exposes existing permission and access controls plus a typed update-policy editor. The central permission filter includes unavailable modules; declarations come from verified published history. Missing signatures or storage faults still fail closed.
...
### Role names and saved organization charts

Role metadata owns chart labels. Renaming a role atomically updates an existing saved chart and increments its organization revision; permission-only edits leave that revision unchanged. Reads project current names over legacy labels, and saves canonicalize labels before persistence. The protected Administrador label remains fixed. Organization keeps conflicted drafts until a successful explicit reload and withholds stale permission previews. See [role-name acceptance](verification/role-names/README.md).
