...
Orders and Inventory retain an explicit `defineTrustedModuleServer` migration bridge for their existing SQL-backed business services. They are reviewed host code; these two bridges still require migration to fully scoped repositories. Trusted code is not a hostile-code sandbox. Independent JSON packages do not execute downloaded JavaScript.
...
The registry currently distributes declarative schemas and metadata. Independently downloadable custom React bundles, a publisher-review portal, per-module migration orchestration remain open work. Multiple reviewed backend versions can now coexist: retain `modules/<id>/releases/<version>/module-server.ts` and its original definition alongside the current entry. Discovery registers each exact version and refuses duplicate identities. Runtime dispatch selects the workspace release and verifies its entire contract against the staged handler. Inventory 1.1 and 1.2 are covered by a simultaneous-pin integration test. This does not replace migration compatibility or executable rollback acceptance. Do not describe the fifth-module test as proof of arbitrary executable plugin support.
...
Migration 012 is additive and preserves existing stock data. Release pins do not make arbitrary resource-schema downgrades safe: per-module migration/rollback compatibility remains a release gate.
