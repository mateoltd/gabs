...
[Linked record reviews](../imported-record-reviews/README.md) subsequently extend all four paths with original-request preservation and current-target conflict choices. Advanced collision/continuation graphs, broader schema/target transitions, delayed denial/profile/expiry/process-death acceptance, larger encrypted corporate archives and recovery from an unreadable existing corporate store remain under ID-03-BACKUP-CORPORATE. Files here are the existing authorized JSON exports, not encrypted bulk backups. No original requirement or overall parity is marked complete.
