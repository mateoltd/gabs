...
## 20 September 2026: imported command dependencies

[ID-03-BACKUP-DEPENDENCIES evidence](imported-command-dependencies/README.md) verifies explicit selection of server-stopped imported dependents without reviving their original requests. One headless browser and one hidden/minimized native journey pass with actual source exports, independent destination stores, child-before-parent import, unchecked copied choices, keyboard selection/reload, selected-only ordering, separate child correction and exactly two records/audit entries after retries. All 867 unit/PostgreSQL tests across 121 files pass, alongside four existing browser command/resource journeys and strict checks/fresh builds. Wide/narrow captures and scoped Axe checks pass. Native OS protection and development identity are controlled fixtures; no CSS changed. Imported record reassignment, broader mixed recovery graphs, encrypted corporate archives, corporate key loss and actual providers/signed platforms remain open. Full parity and UI refinement remain unfinished.
