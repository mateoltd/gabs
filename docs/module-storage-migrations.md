...
New or changed references use the ordinary workspace membership, dependency, resource-read permission and explicit read-grant checks. Referenced records must already exist and be active, so create targets before linking to them. Unchanged historical references may continue to point to archived records. An administrator who introduces cross-module links must also hold the required current business permissions and assignment. Migration permission alone does not grant cross-module access.
...
[Local acceptance evidence](verification/module-migrations/README.md). Complete interrupted client-update recovery, mandatory-update rollout, durable failure dashboards, hosted acceptance and all-platform packaged release acceptance remain tracked separately under EXT-04, EXT-05 and operations work. This manager applies to SDK resource storage; the existing trusted Orders/Inventory SQL bridges remain SDK-01 work.
