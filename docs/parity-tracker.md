...
- **Active:** SDK-01. Both [Orders and Inventory 2.0 candidates](verification/orders-sdk/README.md) now execute their core business mutations through private stores, typed declared services and atomic audits/events/receipts. Signed API tests cover numbering, totals, concurrent reservations, fulfillment, typed failures and prepared imports. Default releases still use the trusted bridges. [Scoped query/read-model acceptance](verification/store-queries/README.md) now covers typed search, ranges, sorting, encrypted cursors and complete aggregates. [Read-only operations](verification/read-only-operations/README.md) now execute fresh queries through web/native transports with consistent per-request snapshots and host-enforced write denial. Next: authoritative relational conversion/reconciliation, current UI/client/worker adapters, complete snapshot exports and explicit role/grant readiness for a coordinated rollout.
...
- **In parallel:** OPS-07 remains open. Remote `35117128232` / `d9c2cc0` passed functional/build, all browser journeys, three unsigned packaging jobs and restore, but measured 622/669 ms p95 for read/confirmation against unchanged 500/1000 ms limits. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 123 unit/PostgreSQL tests in 24 files, strict types/boundaries/copy checks and four builds passed. The five query tests passed again after adding service-only, stale-version and tenant assertions. Four selected headless browser and three minimized/unfocused Electron journeys passed; wide/narrow/native views were inspected. [Read-only query acceptance and limits](verification/read-only-operations/README.md). Remote results above belong to the preceding Orders checkpoint; query checkpoint `891fd14` run `35119858250` is still running, with three unsigned packaging jobs passed.
...
- Remote `35117128232` / `d9c2cc0` passed functional/build/browser/unsigned packaging/restore checks and failed the unchanged read-load target at 650 ms (confirmation 684 ms). OPS-07 remains open.
