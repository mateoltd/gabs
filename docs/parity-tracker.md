...
- **Active:** SDK-01. Both [Orders and Inventory 2.0 candidates](verification/orders-sdk/README.md) now execute their core business mutations through private stores, typed declared services and atomic audits/events/receipts. Signed API tests cover numbering, totals, concurrent reservations, fulfillment, typed failures and prepared imports. Default releases still use the trusted bridges. Next: reusable search/aggregate/read models, authoritative relational conversion/reconciliation, current UI/client adapters and explicit role/grant readiness for a coordinated rollout.
...
- **In parallel:** OPS-07 remains open. Remote `35115231174` / `536b3f2` passed functional/build, all browser journeys, three unsigned packaging jobs and restore, but measured 650/684 ms p95 for read/confirmation against unchanged 500/1000 ms limits. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 112 unit/PostgreSQL tests, strict types/boundaries/copy checks and four builds passed. The CLI exported/checked typed provider contracts and built Orders against an explicit independent Inventory version. All eight selected headless browser journeys and all three minimized/unfocused Electron journeys passed. [Exact candidate acceptance and remaining work](verification/orders-sdk/README.md). The remote full-browser/packaging result belongs to the preceding Inventory checkpoint.
...
- Remote `35115231174` / `536b3f2` passed functional/build/browser/unsigned packaging/restore checks and failed the unchanged read-load target at 650 ms (confirmation 684 ms). OPS-07 remains open.
