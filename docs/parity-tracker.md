...
- **Active:** EXT-05. Explicit optional/mandatory client-release policies, mixed-version server dispatch and existing-dialog controls are implemented. Signed compatible older installations can remain active; mandatory changes preserve queued requests for explicit review. [Evidence and remaining work](verification/module-rollout/README.md). Local policy milestone verification passed. [Receipt recovery across mandatory updates](verification/receipt-recovery/README.md) now preserves exact accepted requests without re-execution; candidate checks passed locally. [Open-editor update recovery](verification/editor-updates/README.md) now retains generated forms and exact uncertain attempts, exports recovery input and protects custom views behind explicit replacement. [Native journal restart and mandatory-update recovery](verification/native-rollout/README.md) now verifies both rejected old-version work and exact committed-receipt recovery in a fresh Electron process. EXT-05 remains active.
- **In parallel:** OPS-07 remains open. Remote run `35091584839` for `dbb13cd` passed code/build checks, all 60 browser journeys, three unsigned packaging jobs and restore, but missed the unchanged read budget at 637 ms (confirmation 692 ms). The next run `35092809379` passed code/build and unsigned packaging, but a stock-filter assertion raced retained loading rows (60/61 browser journeys passed); load and restore were skipped. The test now waits for the result region to finish loading. The earlier 387/420 ms run does not establish consistent acceptance. [Evidence](verification/performance/authorization/README.md).
- **Next:** typed custom-view checkpoint/restore beyond the safe explicit-replacement fallback, unsaved native editor handoff, device rollout observability, partial-failure reporting and connected suspension delivery. Native pending-journal restart across a mandatory update is verified locally for both interrupted and accepted-with-lost-reply requests. Lost-success-reply recovery is verified through the real browser; broader revoked-access/profile recovery remains OFF-03. SDK-01 remains ready. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05; separate administrator data deletion is explicitly EXT-08.
...
- **Last verified baseline:** 76 unit/PostgreSQL tests, 27 distinct selected Chromium journeys, eight distinct Electron journeys, four builds and formatting. New browser checks exercise schema changes, removed input, failed downloads, uncertain receipts and explicit custom-view replacement; native IPC verifies bounded recovery-file export and new native processes recover queued work across mandatory updates. [Current evidence and limits](verification/editor-updates/README.md).
