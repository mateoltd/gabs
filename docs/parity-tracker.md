...
- **Active:** SDK-01. Move the Orders/Inventory transaction bridges onto reusable scoped public SDK capabilities while retaining authoritative concurrency, cross-module atomicity, audit, events and retry behavior. Preserve the current UI and supported pinned-release contracts.
...
- **In parallel:** OPS-07 remains open for consistent candidate acceptance. Remote `35102687893` / `1897f84` passed all 66 browser journeys, build checks, three unsigned packaging jobs, load at 361/418 ms p95 and logical restore. Remote `35104498610` / `ce2887b` passed unit/build/packaging but failed one browser geometry assertion on subpixel rounding; load/restore were skipped. The assertion has been corrected without relaxing the actual layout or performance budgets. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 88 unit/PostgreSQL tests, strict types/boundaries/copy checks and four production builds passed. The final broad browser run passed 66/67; the failing restoration race was fixed, its strengthened delayed-response journey passed three consecutive runs, and all eight affected lifecycle journeys passed. After the final cache-ordering refinement, three focused policy tests and six affected browser journeys passed. All 12 real Electron journeys passed minimized/unfocused on the final application build. [Exact evidence and limits](verification/module-suspension/README.md).
