...
- **Active:** EXT-03, per-module migrations. EXT-02 is verified locally: the official CLI and protected operator console submit, inspect, approve/reject, stage and publish immutable signed releases; independent-server and company-activation acceptance also passed. [Review-console evidence](verification/registry-console/README.md).
- **Then:** EXT-04, schema-safe lifecycle recovery and updates. SDK-01 may proceed independently when useful. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05.
- **Current item:** EXT-03. Repository created at https://github.com/mateoltd/gabs (private), current checkout preserved as `ef70776443ed4020f4c43f9741108d52f9268c58`. Local execution and the Codex goal are active; the user resumed the goal and its active status was verified.
...
- **Last verified baseline:** `933068e` passed unit/build checks, 56 browser journeys and all three unsigned packaging jobs in CI. It failed the p95 order-read gate at 685 ms versus 500 ms; confirmation passed at 525 ms versus 1,000 ms, and restore did not run. The operator console adds 64 passing local unit/PostgreSQL tests and a focused accessible browser journey; see its evidence record. Current performance and hosted release gates remain open.
...
| EXT-03 | active   | Per-module migration manager with namespaced storage, forward compatibility and failure recovery. Exercise interrupted/failed migration without exposing a half-ready release or losing data.                                  | EXT-02                 | BACK-001, BACK-002          |
