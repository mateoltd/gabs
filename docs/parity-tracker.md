...
Updated: 17 September 2026. **Overall status: active; parity has not been achieved.** Owner: the assistant in this project, with user decisions only where materially required.
...
- **Active:** SDK-05. Next: audit public view/worker contexts and privileged host methods, then implement typed declared capabilities and module-scoped grants for desktop/LAN and cross-module standalone access. Check current account/workspace, revocations and stale contexts at the host boundary. React/UI compatibility revisions do not grant authority.
- **Verification:** [Generated response acceptance](verification/generated-response/README.md) passed 235 unit/PostgreSQL tests across 50 files, strict checks/four builds, 20 headless browser journeys and four hidden/unfocused native journeys. SDK-04 is locally verified within its [acceptance map](sdk-04-acceptance.md). Large-registry performance remains OPS-07. The last attempted remote run (`863c59f` / [CI 35190280997](https://github.com/mateoltd/gabs/actions/runs/35190280997)) started no jobs because the Actions budget prevents further use. Full parity and final UI approval remain open.
