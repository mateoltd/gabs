...
- **In parallel:** OPS-07 remains open. [Verified package reuse](verification/performance/verified-content/README.md) reduces repeated content verification/hydration and release selection within read-only snapshots while retaining database authorization checks. Local uninstrumented p95 improved from 410/900 to 170/596 ms against unchanged 500/1,000 ms budgets. Exact candidate remote acceptance is still required. Prior `35145085258` / `ffd7773` passed functional/packaging/restore gates but failed load at 1,529/2,597 ms.
- **Active:** SDK-02. [Coordinated local dependency installation](verification/local-dependencies/README.md) now resolves all active consumers, retains compatible versions, reviews related configuration, commits selected migrations atomically and resumes the complete set offline. Retained dependencies can be restored together. Next: durable download recovery before profile staging, local-profile lifecycle reporting and a retained-release rollback selector. Richer schema/view coverage remains in SDK-04. No external dependency blocks the next engineering item.
...
- **Last verified baseline:** 153 unit/PostgreSQL tests in 30 files, strict types/boundaries/copy checks and four builds passed. Four headless browser journeys exercise scoped onboarding, migrated stock/orders/offline drafts/conflicts, independent signed views/queries and connected suspension after the request-processing change. The preceding local-dependency milestone retains its six browser/three minimized native cases and inspected wide/narrow/native captures. Broader release acceptance and final UI approval remain open. [Current server scope](verification/performance/verified-content/README.md).
