...
- **Active:** SDK-03. [Module-owned scenarios and independent-directory checking](verification/module-scenarios/README.md) are locally verified. [Custom React previews and automatic source/fixture reload](verification/module-preview/README.md) now have headless acceptance too. Next: cross-module fixtures, provider integration and missing simulator capabilities. SDK-04/SDK-05 retain their full authoring/UI/capability scope. OPS-07 continues in parallel; no external dependency blocks the next engineering item.
...
- **Verification:** the last full remote baseline (`17673e8`) passed 160 unit/PostgreSQL tests, code/build, 79 headless browser cases, unsigned packaging and restore; load acceptance failed. The current [SDK-03 preview milestone](verification/module-preview/README.md) passes strict checks and two headless custom-view journeys, with a final source-reload rerun and inspected wide/narrow captures. No native/UI-release acceptance is inferred from these developer-tool checks. Full parity and final UI approval remain open.
...
| SDK-03 | active   | Custom React preview, cross-module fixtures and module-owned test scenarios in the CLI. Invalid types/contracts fail with useful diagnostics; hot reload, permissions and offline simulations remain usable.           | EXT-01 | Typed framework acceptance         |
