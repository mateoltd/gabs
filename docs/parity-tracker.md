...
- **Completed locally:** SDK-03. [Cross-module fixtures and provider development](verification/module-services-preview/README.md) complete the scoped acceptance map for typed module-owned scenarios, custom React preview, source/fixture reload and permission/offline simulation. SDK-04 is active for schema-derived documentation/UI/composability; SDK-05 remains open. The prior remote device-report retry loop is corrected. Fresh remote runs passed 81 browser cases but exposed a fleet-test setup race: dependency removal was attempted before the dependent installed. The corrected setup passes locally; a fresh full remote run remains required. No external dependency blocks this engineering work.
- **Active:** SDK-04. [Typed structured form milestone](verification/schema-forms/README.md) adds public draft/validation helpers, recursive host editors and typed custom-view composition. The form milestone passed 25 focused tests, strict checks/four builds, 21 distinct headless journeys across correction runs and four minimized native journeys. [Schema-derived references](verification/module-documentation/README.md) now have CLI/build parity, drift checks and compiled inferred examples, verified by 20 focused tests and four builds. [Typed resource lists](verification/resource-lists/README.md) now cover inferred equality filters, composable tables, bounded paging, structured values and normalized offline caches, with 50 focused tests, 18 distinct headless journeys across correction runs and a minimized native journey. Recursive reference lookup, richer query controls and complete composability remain in scope; SDK-04 is not complete.
...
- **Verification:** [SDK-04 typed resource lists](verification/resource-lists/README.md) passed strict checks/four builds, 50 focused tests, 18 distinct headless journeys across correction runs and a final minimized native journey. Remote `d6ef4fc` / CI `35161075683` and `6cf68d6` / CI `35162367227` each passed 81 browser cases and all three unsigned packages, then failed activation/fleet test setup assumptions. Activation now verifies disabled invalid input plus independent server rejection; fleet setup resumes the deliberately interrupted dependent before asserting removal denial. Both local corrections passed. Fresh full remote acceptance remains pending; load/restore were not reached. Full parity and final UI approval remain open.
