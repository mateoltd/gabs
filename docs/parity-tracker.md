...
- **In parallel:** OPS-07 remains open. [Verified package reuse](verification/performance/verified-content/README.md) reduced registry work, but latest completed `ce0b275` / CI `35173409882` still missed unchanged 500/1,000 ms budgets at 734/1,593 ms. Code/build, all 194 unit and 87 browser cases, three unsigned packaging jobs and a two-second logical restore passed. Further profiling and optimization are required without weakening authority checks.
...
- **Verification:** [Standalone migration reference reconciliation](verification/local-migration-references/README.md) passed 206 unit/PostgreSQL tests, strict checks/four builds, two distinct headless journeys plus a final capture rerun, and one minimized/unfocused native package recovery journey. Both fresh captures were inspected and historical screenshots preserved. Prior `ce0b275` / [CI 35173409882](https://github.com/mateoltd/gabs/actions/runs/35173409882) passed 194 unit tests, 87 browser journeys, all three unsigned packages and a two-second logical restore, but failed unchanged load budgets at 734/1,593 ms. `844bdff` / CI `35174110106` is still in progress. These remote results verify prior checkpoints, not the new migration changes. OPS-07 remains open. Full parity and final UI approval remain open.
