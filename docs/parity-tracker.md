...
- **In parallel:** OPS-07 remains open. Remote `35145085258` / `ffd7773` passed code/build, all three unsigned packaging jobs, all 74 headless browser cases and a two-second logical restore. Load acceptance failed at 1,529/2,597 ms against unchanged 500/1,000 ms budgets. The preceding `0112be1` run likewise passed functional checks but failed load at 1,490/2,609 ms. [Evidence](verification/local-dependencies/README.md).
...
- **Last verified baseline:** 150 unit/PostgreSQL tests in 29 files, strict types/boundaries/copy checks and four builds passed. Six focused headless browser cases and three minimized/unfocused native cases passed, including multi-module migration failure, restart recovery and an actual dependency/configuration/restoration journey. Wide/narrow/native captures were inspected. Broader release acceptance and final UI approval remain open. [Current scope](verification/local-dependencies/README.md).
