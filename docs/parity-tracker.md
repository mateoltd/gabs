...
- **Active:** EXT-05. SDK clients and generated resource screens now retain the authoring release through queued work, HTTP and Electron. The server rejects mismatched contracts before writes or receipt replay. This is the release-identity foundation, not supported mixed-version dispatch or complete rollout policy. [Evidence and remaining work](verification/client-module-version/README.md).
- **In parallel:** OPS-07 remains open. Run `35057665259` passed at 387/420 ms, but the newer `35057963023` failed reads at 552 ms; confirmation 593 ms and restore passed. Remote profiling identified 750 queries across 50 reads. The current candidate consolidates current identity/membership/role reads into one statement, reducing this to 650 queries without caching authorization. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 75 unit/PostgreSQL tests and rebuilt API, with two browser journeys for the latest stock replay correction. The authorization candidate passed 18 distinct focused Chromium journeys, four builds and formatting. Prior release-identity verification includes all six distinct Electron journeys; both recent remote runs passed all 59 browser journeys and three unsigned packaging jobs. Local p95 reads/confirmation: 102/177 ms. Remote latency and hosted release gates remain open.
