...
- **Active:** SDK-01. The [Inventory 2.0 candidate](verification/inventory-sdk/README.md) now implements product/stock/count/reservation transactions through public SDK stores, service-only operations and declared audits. Its independently signed backend passes real API/SQL acceptance, including concurrent reservations and prepared-snapshot migration rollback. Default Inventory/Orders releases still use their trusted bridges. Next: actual Orders handlers, reusable read models, authoritative relational-data conversion, role/service grants and coordinated rollout. Preserve current UI and supported release contracts.
...
- **In parallel:** OPS-07 remains open. Remote `35112216595` / `6fdc563` passed functional/build checks, all browser journeys, three unsigned packaging jobs and restore, but measured 594/701 ms p95 for read/confirmation against unchanged 500/1000 ms limits. Earlier `1897f84` passed at 361/418 ms; this variation does not establish a performance fix. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 104 unit/PostgreSQL tests, strict types/boundaries/copy checks and four production builds passed. All eight selected browser journeys and all three selected minimized Electron journeys passed. [Exact candidate acceptance and remaining work](verification/inventory-sdk/README.md). Remote full-browser/packaging/restore acceptance belongs to the prior private-store checkpoint; it does not verify later code.
