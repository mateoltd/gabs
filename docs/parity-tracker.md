...
- **Active:** SDK-01. Both [Orders and Inventory 2.0 candidates](verification/orders-sdk/README.md) now execute their core business mutations through private stores, typed declared services and atomic audits/events/receipts. Signed API tests cover numbering, totals, concurrent reservations, fulfillment, typed failures and prepared imports. Default releases still use the trusted bridges. [Scoped query/read-model acceptance](verification/store-queries/README.md) now covers typed search, ranges, sorting, encrypted cursors and complete aggregates. Next: read-only operation dispatch, authoritative relational conversion/reconciliation, current UI/client adapters and explicit role/grant readiness for a coordinated rollout.
...
- **Last verified baseline:** 118 unit/PostgreSQL tests, strict types/boundaries/copy checks and four builds passed. After the locale-search refinement, all 25 affected tests passed, then eight store tests passed with accented-text coverage. Three selected headless browser and three minimized/unfocused Electron journeys passed. [Query acceptance and remaining work](verification/store-queries/README.md). Remote results above belong to the preceding Orders checkpoint.
