...
- **In parallel:** OPS-07 remains open. [Verified package reuse](verification/performance/verified-content/README.md) improved remote p95 to 638/1,598 ms in `d609718` / CI `35148323090`, but still failed unchanged 500/1,000 ms budgets. Code/build, all 76 browser cases, three unsigned packaging jobs and a three-second logical restore passed. Local p95 was 170/596 ms. Further profiling and optimization are required without weakening authority checks.
...
- **Active:** SDK-03. [Module-owned scenarios and independent-directory checking](verification/module-scenarios/README.md) are locally verified. Next: custom React development previews, source/fixture hot reload and cross-module fixtures. SDK-04/SDK-05 retain their full authoring/UI/capability scope. OPS-07 continues in parallel; no external dependency blocks the next engineering item.
...
- **Last verified baseline:** 155 unit/PostgreSQL tests, strict types/boundaries/copy checks and four builds passed. Nine distinct headless browser journeys and three minimized/unfocused native cases passed. Retained code, persisted history, migrations, download recovery and compatibility diagnostics were verified; wide/narrow/native captures were inspected after a stacked-list refinement. Full release acceptance and final UI approval remain open. [Current scope and SDK-02 acceptance map](verification/local-versions/README.md).
