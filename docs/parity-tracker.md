...
- **Verification:** [Signed host UI compatibility](verification/host-ui/README.md) passed 225 unit/PostgreSQL tests across 47 files, strict checks/four builds, five headless browser journeys and one hidden/unfocused native journey on temporary databases. An expanded incompatible-catalog/package recovery rerun also passed. Large-registry loading still exceeded an unchanged timeout in the shared development database; OPS-07 remains open. The last attempted remote run (`655489c` / [CI 35184425890](https://github.com/mateoltd/gabs/actions/runs/35184425890)) started no jobs because the Actions budget prevents further use. Continue local SDK work; remote acceptance, full parity and final UI approval remain open.
