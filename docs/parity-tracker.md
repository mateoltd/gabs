...
- **Verification:** [Reusable resource queries](verification/resource-query/README.md) passed 219 unit/PostgreSQL tests across 46 files on a fresh temporary database, strict checks/four builds, two new headless browser journeys plus ten regression journeys, and two hidden/unfocused native journeys. Final fixture/capture checks are recorded in that evidence file. Historical screenshots and shared development data are preserved. The last attempted remote run (`655489c` / [CI 35184425890](https://github.com/mateoltd/gabs/actions/runs/35184425890)) started no jobs because the Actions budget prevents further use. The last executed run (`39a9b37`) passed 212 unit tests and 90 browser cases but failed the five browser regressions corrected locally in the sorting checkpoint; load/restore were skipped and uploads hit quota. Continue local SDK work; remote acceptance, OPS-07, full parity and final UI approval remain open.
