...
- **Verification:** [Tuple/map editors](verification/map-tuple/README.md) passed 207 unit/PostgreSQL tests on a fresh database, strict checks/four builds, eight headless browser journeys and three hidden/unfocused native journeys. Six captures were inspected and historical screenshots restored. The existing shared-database lifecycle test still times out at its unchanged 30-second limit; the isolated full suite passes. Prior `7634be3` / [CI 35175018089](https://github.com/mateoltd/gabs/actions/runs/35175018089) passed 206 unit tests, 88 browser journeys, all three unsigned packaging commands and a three-second logical restore. It failed load budgets at 813/1,884 ms and artifact uploads at account storage quota. Those remote results verify the previous migration checkpoint, not the new editor changes. OPS-07 remains open. Full parity and final UI approval remain open.
...
| GitHub Actions artifact storage capacity                                          | OPS-04, OPS-05         | CI `35175018089` built all three unsigned packages but upload-artifact rejected every upload for account quota. Preserve acceptance failures; capacity/retention must be resolved before artifact delivery can pass. |
