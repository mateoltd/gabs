...
- **In parallel:** OPS-07 remains open. Remote `35134819192` / `c4460ce` passed code/build and all three unsigned packaging jobs; 69 browser cases passed, but the fresh-company capture timed out because indexed notification locators shifted during dismissal. This continuation waits for natural expiry instead. Load and restore were skipped, so this run supplies no remote latency/recovery acceptance. Earlier measured failures and the 394/863 ms local scoped result remain recorded. [Evidence](verification/business-defaults/README.md).
- **Active:** SDK-02. The [local worker foundation](verification/local-worker/README.md) now supplies typed local handlers/clients, discovered worker-only entry points, cancellation/crash handling, scoped standalone resource transactions, encrypted retry receipts and revision-safe profile writes. Existing local resource editing uses this path in web and hidden Electron. Next: independently signed local executable bundles, installed custom-operation flows and local handler upgrade/recovery acceptance. SDK-03 remains separate CLI/custom preview work. No external dependency blocks the next engineering item.
...
- **Last verified baseline:** 138 unit/PostgreSQL tests in 26 files, strict types/boundaries/copy checks and four builds passed. Worker computation/cancellation, encrypted browser profile receipts/revisions and minimized native restart have scoped acceptance. The preceding SDK-01 clean-database browser acceptance covered 70 distinct cases across the initial run and focused corrections. Broader current-source release acceptance remains open. [Current scope](verification/local-worker/README.md).
