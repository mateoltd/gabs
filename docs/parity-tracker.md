...
- **Active:** OPS-07, verifying the order-list query correction against remote CI with the existing budgets. Local plans identify a badly estimated tenant-wide customer join; indexed customer lookups reduce the observed page query from 21.4 ms to 0.12 ms. [Performance evidence](verification/performance/README.md). EXT-04 is verified locally: durable exact-release attempts, server receipts, interrupted install/update/removal recovery, safe pins, bounded executable storage and real web/native restart journeys. [Lifecycle evidence](verification/module-recovery/README.md).
- **Then:** EXT-05, mandatory-update rollout and lifecycle observability. SDK-01 may proceed independently when useful. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05; separate administrator data deletion is explicitly EXT-08.
- **Current item:** OPS-07. Repository is private at https://github.com/mateoltd/gabs; the initial checkout remains preserved as `ef70776443ed4020f4c43f9741108d52f9268c58`. The user resumed the existing goal and its active status was verified. The UI-refinement goal remains queued after parity.
...
- **Last verified baseline:** 68 unit/PostgreSQL tests, 59 Chromium journeys, 6 Electron journeys, builds, formatting and local logical restore. Local p95 reads/confirmation: 162/183 ms. Remote CI at `101ff85` passed all unsigned packaging jobs but failed p95 reads at 952 ms against 500 ms; confirmation was 720 ms against 1,000 ms and restore did not run. Remote performance and hosted release gates remain open.
...
| OPS-07 | verify | Profile and fix the remote CI read-latency failure without relaxing the 500 ms read / 1,000 ms confirmation budgets. Verify the candidate on the same CI workload and complete its restore gate.                                   | —                                                                                                           | Performance and operations acceptance  |
