...
- **Active:** SDK-01. Both scoped business candidates, typed queries and [coordinated legacy conversion](verification/legacy-business-migration/README.md) now pass signed API/PostgreSQL acceptance. The host reconciles stock, reservations, totals and numbering, converts both modules atomically, preserves source history and fences legacy writes. The default releases still use their trusted bridges. The [worker/export adapter](verification/business-exports/README.md) now reads all pages from one authorized snapshot and delivers scoped Order events. The [business-screen adapter](verification/business-screens/README.md) now supports selected SDK reads/commands, conflict review, offline drafts and exact-version retries in web and hidden Electron. The [administrator cutover](verification/business-cutover/README.md) now reviews explicit role additions and service grants, detects stale policy, applies both migrations atomically and recovers uncertain responses in web and hidden Electron. Next: default public-SDK releases and regression acceptance while preserving historical receipt recovery.
...
- **In parallel:** OPS-07 remains open. Remote `35128117193` / `9ea4364` passed functional/build, browser, three unsigned packaging jobs and restore, but read p95 failed at 725 ms against the unchanged 500 ms limit; confirmation passed at 747 ms against 1000 ms. This used an AMD EPYC 9V74 runner with two cores. Earlier green and failed results remain recorded. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 134 unit/PostgreSQL tests in 24 files, strict types/boundaries/copy checks and four builds passed. Administrator cutover and subsequent business workflows passed in headless Chromium and hidden/minimized Electron. Wide, narrow and native dialog captures were inspected. [Exact scope](verification/business-cutover/README.md).
