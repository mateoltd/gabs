...
- **Verification:** [Read-only query composition](verification/query-context/README.md) passed 226 unit/PostgreSQL tests across 48 files, strict checks/four builds, two headless browser journeys and one hidden/unfocused native journey on temporary databases. Previous [host UI compatibility acceptance](verification/host-ui/README.md) covers signed requirements and data-preserving update recovery. The [SDK-04 map](sdk-04-acceptance.md) keeps returned resource record/page validation open. Large-registry performance remains OPS-07. The last attempted remote run (`3f4507f` / [CI 35186225632](https://github.com/mateoltd/gabs/actions/runs/35186225632)) started no jobs because the Actions budget prevents further use. Full parity and final UI approval remain open.
