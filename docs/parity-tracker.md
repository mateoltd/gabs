...
- **Last verified baseline:** 129 unit/PostgreSQL tests in 24 files, strict types/boundaries/copy checks and four builds passed. Two selected headless browser and three hidden/minimized Electron journeys passed. The local five-second logical restore checks migrated private-store balances, reservations, order totals/states, numbering, RLS and the retired-write fence. [Exact conversion evidence and remaining work](verification/legacy-business-migration/README.md).
