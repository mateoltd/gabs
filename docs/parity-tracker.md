...
- **Active:** SDK-02. [Durable local installation recovery](verification/local-install-recovery/README.md) now saves verified package/configuration before migration, resumes offline after browser/native interruption, records acceptance atomically and refuses stale accepted replay. Explicit discard preserves the installed module and data. [Local schema migration and rollback](verification/local-migrations/README.md) remain covered. Next: coordinated local dependency installation/update, local-profile lifecycle reporting and a retained-release rollback selector. Richer schema/view coverage remains in SDK-04. No external dependency blocks the next engineering item.
...
- **Last verified baseline:** 147 unit/PostgreSQL tests in 28 files, strict types/boundaries/copy checks and four builds passed. Four focused headless browser cases and three minimized/unfocused native cases passed, followed by focused acceptance for the wrapping recovery list and native resume interface. Wide/narrow/native recovery captures were inspected; scoped Axe checks pass. Broader release acceptance and final UI approval remain open. [Current scope](verification/local-install-recovery/README.md).
