...
- **In parallel:** OPS-07 remains open. Remote `35139112513` / `78206ad` passed code/build, all three unsigned packaging jobs and 71 of 72 browser cases. The fleet test read a downloading observation before the failure report was acknowledged; its corrected journey now waits for the relevant server reports and passes locally. Load and restore were skipped. Earlier remote latency failures and the 394/863 ms local scoped result remain recorded. [Evidence](verification/local-controls/README.md).
- **Active:** SDK-02. [Local module controls and operation recovery](verification/local-controls/README.md) now cover personal-registry installation, independent local configuration, offline custom operations, cancellation, rejected work, restart recovery and data-preserving removal/reinstallation through web/native UI. Exact requests are encrypted before execution; records, acceptance state and receipts commit atomically. Local defaults are isolated from corporate runtime registration. Next: reviewed local schema migrations/recovery, coordinated dependency installation/update, offline reactivation of retained modules and local-profile lifecycle reporting; arbitrary schema/view coverage remains in SDK-04. No external dependency blocks the next engineering item.
...
- **Last verified baseline:** 141 unit/PostgreSQL tests in 27 files, strict types/boundaries/copy checks and four builds passed. Four focused headless browser cases and three minimized/unfocused native cases passed. New local action/module dialogs were inspected at wide/narrow sizes, with scoped Axe checks. Broader release acceptance and final UI approval remain open. [Current scope](verification/local-controls/README.md).
