...
- **Active:** EXT-05. Explicit optional/mandatory client-release policies, mixed-version server dispatch and existing-dialog controls are implemented. Signed compatible older installations can remain active; mandatory changes preserve queued requests for explicit review. [Evidence and remaining work](verification/module-rollout/README.md). Local policy milestone verification passed. [Receipt recovery across mandatory updates](verification/receipt-recovery/README.md) now preserves exact accepted requests without re-execution; candidate checks passed locally. [Open-editor update recovery](verification/editor-updates/README.md) now retains generated forms and exact uncertain attempts, exports recovery input and protects custom views behind explicit replacement. [Native journal restart and mandatory-update recovery](verification/native-rollout/README.md) now verifies both rejected old-version work and exact committed-receipt recovery in a fresh Electron process. [Typed custom-view checkpoint/restore](verification/custom-view-state/README.md) now validates live input, supports explicit schema conversion and restores the previous view after an initial render failure. [Generated native editor handoff](verification/native-editor-updates/README.md) now verifies unsaved input, removed resources, native export and exact uncertain-receipt recovery. [Device rollout visibility](verification/module-fleet/README.md) now separates server receipts from device progress/failures, with scoped durable reports and a paginated administrator view. EXT-05 remains active.
- **In parallel:** OPS-07 remains open for consistent candidate acceptance. Remote `35102687893` / `1897f84` passed all 66 browser journeys, build checks, three unsigned packaging jobs, load at 361/418 ms p95 and logical restore. Earlier runs missed the unchanged 500 ms read budget at 514–761 ms; this passing observation does not establish a performance fix across runner variation. [Evidence](verification/performance/authorization/README.md).
- **Next:** connected suspension delivery with emergency/offline acceptance. [Report recovery](verification/report-recovery/README.md) now covers durable bounded retries, preflight failures and removal receipts. Typed custom-state conversion and preservation now have real browser/Electron evidence; arbitrary unregistered component state and durable custom-operation recovery are not implicitly complete. Native pending-journal restart across a mandatory update is verified locally for both interrupted and accepted-with-lost-reply requests. Lost-success-reply recovery is verified through the real browser; broader revoked-access/profile recovery remains OFF-03. SDK-01 remains ready. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05; separate administrator data deletion is explicitly EXT-08.
...
- **Last verified baseline:** 81 unit/PostgreSQL tests and four production builds. The full 66-browser selection initially passed 63 and exposed three issues; all 11 affected browser journeys passed after corrections, including the three failures. All 11 real Electron journeys passed minimized/unfocused, and formatting passed. [Current evidence and limits](verification/module-fleet/README.md).
...
| EXT-05 | active   | Mandatory-update rollout controls and lifecycle observability. Demonstrate pinned-client compatibility, emergency suspension, partial rollout failure and explicit offline lease limits.                                       | EXT-04                 | ORG-004                        |
