...
- **Next:** typed custom-view checkpoint/restore beyond the safe explicit-replacement fallback, then native transport interruption/restart with mixed-version queued work, device rollout observability, partial-failure reporting and connected suspension delivery. Lost-success-reply recovery is verified through the real browser; broader revoked-access/profile recovery remains OFF-03. SDK-01 remains ready. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05; separate administrator data deletion is explicitly EXT-08.
...
- **Last verified baseline:** 76 unit/PostgreSQL tests, 27 distinct selected Chromium journeys, all six Electron journeys, four builds and formatting. New browser checks exercise schema changes, removed input, failed downloads, uncertain receipts and explicit custom-view replacement; native IPC verifies bounded recovery-file export. [Current evidence and limits](verification/editor-updates/README.md).
