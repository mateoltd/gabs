...
- **Last verified baseline:** 79 unit/PostgreSQL tests, 27 distinct selected Chromium journeys across candidate iterations (seven affected journeys rerun after the uninstall fix), eleven distinct Electron journeys across the implementation and subsequent test-only addition, four builds and formatting. Typed custom input survives compatible release changes, explicit conversion and initial-render rollback; stale background installers respect completed uninstall. All native journeys ran minimized without focus. [Current evidence and limits](verification/custom-view-state/README.md).
