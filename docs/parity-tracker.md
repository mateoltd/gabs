...
- **In parallel:** OPS-07 remains open. Remote `35136781563` / `3d42f93` passed code/build, all three unsigned packaging jobs and 70 of 71 browser cases. The upgrade retry test raced a live completion event; this continuation deterministically delays completion reads and verifies the exact manual retry. Load and restore were skipped. Earlier remote latency failures and the 394/863 ms local scoped result remain recorded. [Evidence](verification/local-executables/README.md).
- **Active:** SDK-02. [Independent local executables](verification/local-executables/README.md) now build, receive registry review/publication, install through authenticated device administration and execute from encrypted local profiles without a server component or host edits. Real browser and packaged native workers verify offline execution, compatible upgrades, exact historical receipts and retained data after uninstall/reinstall. Next: user-facing local installation/configuration/removal, custom-operation/cancellation/recovery controls and local schema migration support. SDK-03 remains separate CLI/custom preview work. No external dependency blocks the next engineering item.
...
- **Last verified baseline:** 140 unit/PostgreSQL tests in 27 files, strict types/boundaries/copy checks and four builds passed. Three focused browser cases and two minimized/unfocused desktop cases verify signed local execution/lifecycle and upgrade retry recovery. Broader current-source release acceptance remains open. [Current scope](verification/local-executables/README.md).
