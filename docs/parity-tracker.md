...
- **In parallel:** OPS-07 remains open after remote run `35056056304`: read p95 670 ms exceeds 500 ms; confirmation 726 ms passes 1,000 ms. CPU/query diagnostics now run after a failed CI load gate without replacing its result; restore runs independently after load. [Performance evidence](verification/performance/README.md).
