...
- **Active:** EXT-04, schema-safe lifecycle recovery and updates. EXT-03 is verified locally: signed scoped migration handlers, atomic failure/crash recovery, current authorization, schema-compatible executable selection and administrator migration/retry controls. [Migration evidence](verification/module-migrations/README.md).
- **Then:** EXT-05, mandatory-update rollout and lifecycle observability. SDK-01 may proceed independently when useful. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05.
- **Current item:** EXT-04. Repository created at https://github.com/mateoltd/gabs (private), current checkout preserved as `ef70776443ed4020f4c43f9741108d52f9268c58`. Local execution and the Codex goal are active; the user resumed the goal and its active status was verified.
...
- **Last verified baseline:** local migration acceptance passed 66 unit/PostgreSQL tests, 58 Chromium journeys, 5 Electron journeys, all builds, formatting and logical restore with schema-history/RLS checks. Local p95 reads/confirmation: 150/233 ms. Remote CI at `e5be7b0` passed all unsigned packaging jobs but failed p95 order reads at 925 ms against 500 ms; confirmation was 699 ms against 1,000 ms and restore did not run. Remote performance and hosted release gates remain open.
...
| EXT-04 | active   | Resumable installation, update, pins, repair, suspension and schema-safe executable rollback acceptance. Preserve data on uninstall and block incompatible dependency/backend/schema combinations with actionable diagnostics. | EXT-01, EXT-03         | CORE-002, ORG-004           |
