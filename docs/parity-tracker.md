...
- **In parallel:** OPS-07 remains open. Remote `35130858941` / `2bde7ab` passed functional/build, browser, three unsigned packaging jobs and restore, but read p95 failed at 598 ms against 500 ms; confirmation passed at 630 ms against 1000 ms. The subsequent scoped-default workload initially failed locally at 494/1067 ms. Dependency-closure registry reads improved the final local result to 394/863 ms without security caching or relaxed limits. Current remote acceptance remains required. [Evidence](verification/business-defaults/README.md).
- **Next primary:** SDK-02, standalone custom operations and substantial computation in workers. The local profile currently edits standalone resources directly in the renderer; custom local operations, crash/cancel semantics and worker capability boundaries need implementation. SDK-03 remains separate CLI/custom preview work. Broader working-set/profile recovery remains OFF-02/03, hosted trust/release acceptance EXT-06 and OPS-04/05, and separate administrator data deletion EXT-08. No external dependency blocks SDK-02.
...
- **Last verified baseline:** 135 unit/PostgreSQL tests in 25 files, strict types/boundaries/copy checks and four builds passed. Fresh-company and reviewed-upgrade workflows passed in headless Chromium and hidden/minimized Electron. Fresh-database browser acceptance covered 70 distinct cases: 68 initial passes plus two focused passes after correcting legacy seed-number selectors. Local scoped load and initialization-aware logical restore passed; captures were inspected without changing layout/styles. [Exact scope](verification/business-defaults/README.md).
