...
- **Active:** SDK-02. [Durable local downloads](verification/local-downloads/README.md) preserve each verified package before installation, recover across reload/native restart and fetch only missing packages with current source-account access. Completed sets review/install offline; installation staging consumes the download atomically. Next: local lifecycle history/reporting and a retained-release rollback selector. Richer schema/view coverage remains in SDK-04. No external dependency blocks the next engineering item.
...
- **Last verified baseline:** 153 unit/PostgreSQL tests, strict types/boundaries/copy checks and four builds passed. Eight headless browser cases and three distinct minimized/unfocused native cases passed; a final scroll fix was rechecked with two browser and two native cases. Inspected wide/narrow/native download recovery and installed work. Full release acceptance and final UI approval remain open. [Current scope](verification/local-downloads/README.md).
