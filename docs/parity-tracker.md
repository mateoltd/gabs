...
- **In parallel:** OPS-07 remains open. Remote `35124732655` / `33ca122` and `35125719424` / `a906842` passed functional/build, browser, three unsigned packaging jobs and restore, but read p95 failed at 691 ms and 644 ms against the unchanged 500 ms limit; confirmations passed at 721/800 ms against 1000 ms. Both used AMD EPYC 7763 runners. Earlier green and failed results remain recorded. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 130 unit/PostgreSQL tests in 24 files, strict types/boundaries/copy checks and four builds passed. Three headless browser journeys and three hidden/minimized Electron checks passed; the expanded migrated product-edit/count and offline-retry journey passed separately. Wide, narrow and native captures were inspected. [Exact scope](verification/business-screens/README.md).
