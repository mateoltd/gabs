...
- **Active:** SDK-01. Both scoped business candidates, typed queries and [coordinated legacy conversion](verification/legacy-business-migration/README.md) now pass signed API/PostgreSQL acceptance. The host reconciles stock, reservations, totals and numbering, converts both modules atomically, preserves source history and fences legacy writes. The default releases still use their trusted bridges. The [worker/export adapter](verification/business-exports/README.md) now reads all pages from one authorized snapshot and delivers scoped Order events. Next: current business API/client/UI adapters, administrator permission/grant review and the coordinated cutover flow.
...
- **In parallel:** OPS-07 remains open for current-candidate/deployed acceptance and runner variation. Remote `35121357831` / `eaf5308` passed functional/build, all browser journeys, three unsigned packaging jobs, restore and load at 471/528 ms read/confirmation p95 against unchanged 500/1000 ms limits. This predates the legacy-conversion changes; earlier failures remain recorded. [Evidence](verification/performance/authorization/README.md).
...
- **Last verified baseline:** 130 unit/PostgreSQL tests in 24 files, strict types/boundaries/copy checks and four builds passed. After the final worker loop adjustment, 42 affected tests passed. One headless browser journey verified actual CSV creation, background processing and download. A six-second logical restore passed all nine invariants across 34 migrated SDK workspaces. [Export checkpoint evidence](verification/business-exports/README.md).
