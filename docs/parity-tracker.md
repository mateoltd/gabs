...
- **In parallel:** OPS-07 remains open. Latest completed remote runs `35099113592` / `381c0eb` and `35099603873` / `d029dbd` passed functional/build, three unsigned packaging jobs and logical restore, but read p95 was 514/515 ms (confirmation 855/576 ms) on AMD EPYC 9V74. Earlier EPYC 7763 runs missed reads at 640–761 ms. The 500/1000 ms budgets remain unchanged; runner variation is not a latency fix. [Evidence](verification/performance/authorization/README.md).
- **Next:** complete report delivery/backoff and preflight/removal failure coverage, then connected suspension delivery with emergency/offline acceptance. Typed custom-state conversion and preservation now have real browser/Electron evidence; arbitrary unregistered component state and durable custom-operation recovery are not implicitly complete. Native pending-journal restart across a mandatory update is verified locally for both interrupted and accepted-with-lost-reply requests. Lost-success-reply recovery is verified through the real browser; broader revoked-access/profile recovery remains OFF-03. SDK-01 remains ready. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05; separate administrator data deletion is explicitly EXT-08.
