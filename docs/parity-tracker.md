...
- **Active:** EXT-05. Explicit optional/mandatory client-release policies, mixed-version server dispatch and existing-dialog controls are implemented. Signed compatible older installations can remain active; mandatory changes preserve queued requests for explicit review. [Evidence and remaining work](verification/module-rollout/README.md). Local milestone verification passed; EXT-05 remains active.
- **In parallel:** OPS-07 remains open. The newest remote run `35087635112` failed the unchanged read budget at 689 ms; confirmation 698 ms and restore passed. The preceding candidate failed at 678/679 ms. The earlier 387/420 ms run does not establish consistent acceptance. [Evidence](verification/performance/authorization/README.md).
- **Next:** finish safe active-editor handoff and lost-success-reply recovery across mandatory updates (EXT-05/OFF-03), then device rollout observability, partial-failure reporting, connected suspension delivery and native mixed-version acceptance. SDK-01 remains ready. Hosted trust/production release acceptance remain open in EXT-06 and OPS-04/05; separate administrator data deletion is explicitly EXT-08.
...
- **Last verified baseline:** 76 unit/PostgreSQL tests, 16 focused Chromium journeys, all six Electron journeys, four builds and formatting. New policy controls passed Axe checks and wide/narrow visual inspection. This does not close full release, native mixed-version, arbitrary unsaved-editor or remote performance gates. [Current milestone evidence](verification/module-rollout/README.md).
