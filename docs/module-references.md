...
For cross-module links introduced by a migration, review exact consumer/provider releases in the local installer. Explicit consent is saved with the recoverable installation; the active grants and all module data commit together. Providers migrate before consumers, and the shared worker broker supplies only permitted final provider resources for reference validation. Rejected or cancelled installations preserve existing releases, records and permissions. Unlock/retry retains consent, while explicit revocation removes matching pending decisions. See [acceptance](verification/local-migration-grants/README.md). Local migration handlers still cannot invoke arbitrary provider services or mutate another module directly.
...
- [Cross-module standalone reference lookup](verification/local-reference-grants/README.md) now has web/native consent acceptance. Custom standalone view mounting and local service transactions remain open. [Installation-time reference consent](verification/local-migration-grants/README.md) now supplies verified final provider snapshots for new migration links.
- In-memory simulation does not establish real corporate authorization, SQL isolation or concurrency. Local migration reconciliation is validated in the worker/profile lifecycle; it does not implement corporate import, local device capabilities or arbitrary cross-module migration services.
