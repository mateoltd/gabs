...
The durable broker, browser/native effect adapters and owner recovery controls have scoped local acceptance. [Standalone development simulation](local-device-simulation.md) now has SDK, CLI and preview acceptance. Corporate offline capability leases, actual OS notification presentation and positive corporate native LAN acceptance remain open. No module operation should present a queued device request as a completed effect.
