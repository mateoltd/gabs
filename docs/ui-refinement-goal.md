...
- Reopening an imported draft under a changed schema shows the obsolete field twice, in “Fields removed by update” and “Unsupported saved fields,” with two removal controls. Consolidate the presentation while retaining explicit removal and source preservation. The restoration success notice should also explain when a resource is retired instead of directing users to an unavailable editor. [Web/native schema and retired-resource captures](verification/imported-schema/README.md), 20 September 2026. This observation does not start refinement or grant design approval.
