...
Standalone profiles retain encrypted IndexedDB vaults and can wrap their key with an optional PIN. Desktop quick-unlock envelopes additionally use OS protection, with purpose-bound biometric prompts. The current standalone runtime receives the unwrapped key through a narrow capability and still performs vault cryptography in the renderer; utility-process custody and full-file persistence for standalone profiles remain ID-03 implementation work. Passphrase recovery remains available and removal clears quick unlock without deleting encrypted business data. See [standalone unlock acceptance](verification/local-profile-unlock/README.md).
