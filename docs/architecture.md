...
Standalone profiles share a portable vault engine behind platform-specific storage and access contracts. Browsers retain encrypted IndexedDB vaults. Desktop profiles use the page-encrypted utility database; cryptographic keys stay in the utility process and OS wrapping stays in main. The renderer receives revocable handles through a typed operation protocol. Existing desktop IndexedDB envelopes migrate with transactional receipts and conditional source cleanup, so replay cannot overwrite newer native work. Passphrase recovery and optional PIN wrapping remain available; removal clears quick unlock without deleting business data. Desktop access requires the OS-protected database key, including when unlocking by passphrase. Lost-key recovery, durable rotation and actual OS-provider acceptance remain ID-03 gates. See [native vault acceptance](verification/native-vaults/README.md).
