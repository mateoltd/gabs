...
Electron loads packaged assets with Node integration disabled, context isolation and sandboxing enabled. A named operation allowlist replaces arbitrary authenticated HTTP or filesystem access. Login, tokens, PKCE and refresh live in main. Tokens and the cache master key use OS-backed safeStorage encryption, with session-only authentication credential fallback if unavailable. Corporate cache pages and WAL use authenticated page encryption in the utility process, with a purpose-derived key and retained AES-GCM payload protection; the database fails closed if its protected key is missing. See [migration and storage acceptance](verification/protected-database/README.md). Corporate authentication credentials are not exposed through preload. Local pending operations may remain volatile in that fallback.
