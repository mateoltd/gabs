...
Electron loads packaged assets with Node integration disabled, context isolation and sandboxing enabled. A named operation allowlist replaces arbitrary authenticated HTTP or filesystem access. Login, tokens, PKCE and refresh live in main. Tokens and the cache master key use OS-backed safeStorage encryption, with session-only authentication credential fallback if unavailable. Corporate cache pages and WAL use authenticated page encryption in the utility process, with a purpose-derived key and retained AES-GCM payload protection; the database fails closed if its protected key is missing. See [migration and storage acceptance](verification/protected-database/README.md). Protected files share a main-owned, serialized persistence adapter with durable atomic replacement and provider-requested envelope renewal; see [key-file acceptance](verification/protected-key-renewal/README.md). Corporate authentication credentials are not exposed through preload. Local pending operations may remain volatile in that fallback.

Standalone profiles share a portable vault engine behind platform-specific storage and access contracts. Browsers retain encrypted IndexedDB vaults. Desktop profiles use the page-encrypted utility database; cryptographic keys stay in the utility process and OS wrapping stays in main. The renderer receives revocable handles through a typed operation protocol. Existing desktop IndexedDB envelopes migrate with transactional receipts and conditional source cleanup, so replay cannot overwrite newer native work. Passphrase recovery and optional PIN wrapping remain available; removal clears quick unlock without deleting business data. Successful native quick unlock [renews both credential wrappers atomically](verification/vault-key-renewal/README.md); renewal returns ciphertext only and does not replace biometric authorization. Desktop access requires the OS-protected database key, including when unlocking by passphrase. Lost-key recovery, durable rotation and actual OS-provider acceptance remain ID-03 gates. See [native vault acceptance](verification/native-vaults/README.md).
