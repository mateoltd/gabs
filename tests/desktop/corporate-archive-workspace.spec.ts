...
import { archiveWorkspace } from "../support/corporate-portability/archive-workspace";
...
test("archive previews and admission remain isolated through workspace changes", async () => {
  test.setTimeout(240000);
  const directory = await mkdtemp(
    resolve(tmpdir(), "suite-archive-workspace-"),
  );
  ...
  try {
    ...
    await corporatePortability({
      ...
      archiveReviewCheck: (context) =>
        archiveWorkspace(context, { surface: "native", api }),
      source: device.page,
      api,
      directory,
