...
import { roleRemovalJourney } from "../support/role-removal-journey";
import {
  test,
  expect,
...
test("hidden native administrators recover role removal and preserve history", async () => {
  test.setTimeout(90000);
  const profile = await mkdtemp(resolve(tmpdir(), "suite-role-removal-"));
