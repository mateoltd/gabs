...
test("desktop renews the protected database-key envelope and preserves standalone work after controlled provider-key retirement", async () => {
  ...
  const launch = async (key: string, previousKey?: string) => {
    ...
    try {
      ...
      await controlledNativeProtection(app, key, previousKey);
      return app;
    } catch (error) {
      await app.close();
      throw error;
