...
import { groupAdministrationJourney } from "../support/group-administration-journey";
import {
  test,
  expect,
...
test("hidden native administrators manage groups and permission decisions", async () => {
  test.setTimeout(90000);
  const profile = await mkdtemp(resolve(tmpdir(), "suite-groups-"));
  ...
  try {
    ...
    await groupAdministrationJourney(
      page,
