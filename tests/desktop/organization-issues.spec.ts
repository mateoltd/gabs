...
import { organizationIssuesJourney } from "../support/organization-issues-journey";
import {
  test,
  expect,
...
test("hidden native administrators diagnose and repair invalid organization drafts", async () => {
  test.setTimeout(90000);
  const profile = await mkdtemp(
    resolve(tmpdir(), "suite-organization-issues-"),
  );
  ...
  try {
    ...
    await organizationIssuesJourney(
      page,
