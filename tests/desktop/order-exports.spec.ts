...
test("hidden Orders exports use server bytes and reject post-dialog revocation, stale views and generic CSV writes", async () => {
  ...
  try {
    ...
    await expect
      .poll(() =>
        app!.evaluate(
          () =>
            (globalThis as unknown as { orderExport: DialogState }).orderExport
              .waiting,
        ),
      )
      .toBe(true);
    ...
    expect(await audits()).toBe(before + 1);
    await mkdir("docs/verification/order-export-delivery", { recursive: true });
    await page.screenshot({
      path: "docs/verification/order-export-delivery/native.png",
    });
    ...
    await app.evaluate((_, path) => {
      (globalThis as unknown as { orderExport: DialogState }).orderExport.path =
        path;
    }, abandoned);
    await download.click();
    await expect
