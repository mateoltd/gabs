...
import { organizationScaleJourney } from "../support/organization-scale-journey";
import {
  test,
  expect,
...
test("hidden native administrators manage a large organization", async () => {
  test.setTimeout(180000);
  const profile = await mkdtemp(resolve(tmpdir(), "suite-organization-scale-"));
  ...
  try {
    ...
    await organizationScaleJourney(
      page,
