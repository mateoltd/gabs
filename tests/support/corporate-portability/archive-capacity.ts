...
export async function archiveCapacity(options: {
  ...
  const names = [
    ...
    "Large note A " + "a".repeat(560000),
    "Large note B " + "b".repeat(560000),
  ];
  ...
  await archive()
    ...
    .click();
  await expect(archive().getByRole("checkbox")).toHaveCount(35);
  ...
  expect(decoded.copies).toHaveLength(35);
  ...
  await add([32]);
  await expectCount(32);
  await restoreAndRemove(1);
  await add([33]);
  await expectCount(32);
  await restoreAndRemove(2);
