...
for (const choice of ["original"] as const)
  test(`switches real imported snapshots with ${choice} target and retained reviews`, async ({
    browser,
  }) => {
    ...
    try {
      ...
      await corporateRequestTargets({
        ...
        snapshots: true,
        choice,
        draftReference: choice === "original" ? "reassigned" : "original",
        directory,
        evidenceName: "web",
