...
import { promotionExpiry } from "../support/corporate-portability/promotion-expiry";

for (const surface of ["web", "desktop"] as const)
  test(`${surface} restoration requires fresh sign-in when server recovery authority expires during settlement`, async ({
    browser,
  }) => {
    test.setTimeout(240000);
    const directory = await mkdtemp(
      resolve(tmpdir(), "suite-promotion-expiry-"),
    );
    ...
    try {
      ...
      await corporatePortability({
        ...
        confirmRestoration: async (context) => {
          ...
          return promotionExpiry(context, {
            surface,
