  ...
  test(
    ...
    async ({ page, context }) => {
      ...
      try {
        ...
        await dialog
          .getByRole("button", { name: "Save update policy" })
          .click();
        ...
        if (lostReply) {
          ...
          await page.route(
            ...
            async (route) => {
              if (!first || route.request().postDataJSON()?.action !== "create")
        ...
        if (lostReply) {
          ...
          const recovered = (await journal())[0];
          expect(recovered).toMatchObject({
            id: pending.id,
            call: pending.call,
            state: "accepted",
          });
          expect(recovered.supersededBy).toBeUndefined();
          ...
          await mkdir("docs/verification/receipt-recovery", {
            recursive: true,
          });
          await page.screenshot({
            path: "docs/verification/receipt-recovery/recovered.png",
          });
          await page.setViewportSize({ width: 390, height: 844 });
          expect(
            await page.evaluate(
          ...
          await page.screenshot({
            path: "docs/verification/receipt-recovery/narrow.png",
          });
          return;
        }
        await expect(
