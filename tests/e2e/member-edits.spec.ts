...
test("administrators resolve stale access and retry an accepted edit without duplication", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await memberEditsJourney(page, await createReviewWorkspace(send), send);
});
