...
test("administrators retry accepted invitation creation and revocation without duplication", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await invitationRetriesJourney(page, await createReviewWorkspace(send), send);
});
