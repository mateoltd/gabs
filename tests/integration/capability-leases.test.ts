...
it.each(["files.export", "lan.status", "lan.relay"] as const)(
  "issues audited exact-release %s leases and rechecks revoked authority before idempotent replay",
  async (kind) => {
    ...
    try {
      ...
      expect(lease.payload.expiresAt - lease.payload.issuedAt).toBe(
        24 * 3600000,
      );
      const context = {
        ...
        call: {
          ...
          input:
            kind === "files.export"
              ? { filename: "cached.txt", content: "Cached data" }
              : kind === "lan.status"
                ? {}
                : {
                    peerId: "trusted-peer",
      ...
      expect((await acquire()).json().code).toBe(
