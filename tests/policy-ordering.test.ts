...
import {
  newerPolicy,
  snapshotWithPolicy,
} from "../packages/app-web/src/policy-delivery";
...
it("does not restore older permission or lease snapshots after a newer policy arrives", () => {
  ...
  const restored = snapshotWithPolicy(disk, suspended);
  expect(restored.bootstrap).toBe(suspended);
  expect(restored.expiresAt).toBe(Date.parse(suspended.authorizedAt));
  expect(snapshotWithPolicy(restored, before).bootstrap).toBe(suspended);
  expect(snapshotWithPolicy(disk)).toBe(disk);
});
